...
//   - golden-calibration.js → both, against the pre-scored golden samples

/** System prompt for the writing evaluator (/api/correct). `safePrompt` must
 *  already be sanitized by the caller (sanitizeParam). */
export function writingEvalSystemPrompt(safePrompt) {
  return `You are a Croatian language teacher. The student was asked to write about: "${safePrompt}".
...
/** User prompt for the speaking rubric (/api/assess-speaking). `level`,
 *  `prompt` and `transcript` must already be sanitized by the caller. */
