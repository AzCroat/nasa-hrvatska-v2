...
- **Serials survive a reader who can open any part first.** Every part opens
  readable on its own, its title ends "(k/n)", and its English `intro` says
  ...
  tap. (The daily "Read" row is a different corpus: `dailyInput` →
  `readingCurriculum` walks the `READ` pool in `exercises.js`, not
  `GRADED_STORIES`.) **A constraint whose justification has died reads as
  arbitrary, and the next author deletes it.** The catalog projection (`/api/content/catalog`) does
...
- **WHAT THE "20 OF 40" MEASURES, STATED PRECISELY (2026-09-05, found while
  ...
  that branch; that is the branch it measures. City of the Day itself is
  English prose plus three Croatian words per city (364 cities, no Croatian
  text field), which is why levelling it is a content job measured in tens of
  thousands of words, not a rotation fix.
  **OWNER DECISION (2026-09-05, same day): the first claim is A1–A2 only.**
  `CITY_OF_DAY_SLOT_MAX_CEFR = 'A2'` (`croatiaPool.ts`). From B1 up the slot is
  the level-aware rotation on EVERY daily build and `cityofday` is not in the
  eligible pool at all — an advanced learner's culture day is own-tier essays,
  news, regions and history, alternating with the lower tier as designed. City
  of the Day stays on Home (`CityOfDayCard`) for everyone. B1 is the threshold
  because it is the first level with a substantial own tier (15 B1 entries plus
...
Measured: **1,869 Croatian strings outside TARGETS, of which the widened matcher already sees 69%** — against 12% before the third wave, which is the third wave working. **55 files sat at ratio ≥ 0.5 and a sweep of all of them reported ZERO findings**, so the 54 that were added (466 → 525, +1,849 strings scanned, census 112,738 → 114,587) bought a **ratchet against future edits, not a bug fix today**. Say that plainly rather than dressing a clean sweep up as a save: the value is that `DiasporaNote.tsx` — where `šerati` actually lived in 2026-08-26 — is now scanned, mutation-verified in both directions (a Serbism in it is caught; with the file removed from TARGETS the same Serbism passes clean).
...
## Critical Architecture: A Test Can Keep A Dead Screen Alive (2026-09-25)

`noUnreachableModules.test.ts` seeds its reachability walk from every TEST as well
as the app entries, deliberately: "a module kept alive only by its own tests still
counts as reachable — that is a softer problem and is not what this guard is for."
True of a helper with a unit test. Measured against the tree, that exclusion was
...
- **THE COST WAS WORK ON THE WRONG FILE, TWICE, BY THE AUDIT ITSELF.** #655
  ...
  anyone anything. Those entries above are corrected in place rather than rewritten.
- **`#682`'s sweep deleted 31 modules of exactly this kind and could not see
  these**, because these have tests. So this set is the residue of that sweep,
  hidden by its own seeding rule — a guard's stated scope is also its blind spot,
...
- **One dead module sits inside a LIVE directory**, which is the hazard that guard
  ...
  class and root, `verbsData.test.ts` asserts the STORED forms equal the
  derivation, and the app renders those stored forms through `forms.ts`'s
  `formFor`, which is a LOOKUP rather than a competing rule. The data a learner
  meets is exactly what was validated. It is a test-only VALIDATOR — the "softer
  problem" the guard's docstring actually means — and it is listed because
  membership must never be silent, not because anything is wrong.
- **Three are dead CLUSTER members**, reachable only through another dead module —
  `useLocalStorage` (via `useHeroRewards`), `useTranslator` (via
  `useKnightSpeech`), `storyRecommendation` (via `StoryOfTheDayCard`) — which is
  why "no importer" could never have found them and only a reachability walk can.
- **The mechanism is a second walk in the same file**, seeded from `ENTRIES` ONLY,
  requiring every app-unreachable module to be named in `TEST_ONLY_REACHABLE` with
  a reason, both staleness directions checked. Membership is not automatically a
  defect — a library with a test and no caller yet is not a 389-line screen — but
  it can no longer be SILENT, which is the whole of what went wrong.
...
| ------------------------------------------- | ------------------------------------------------------------------------------------------ |
| `HomeTab` / `SessionCard` / `QuestTracker`  | `home.spec.js`, `daily-challenge-sync.spec.js`, `profile-persist.spec.js`                  |
...
- **CodeQL analyses the branch we DEPLOY, not only PRs** (2026-09-22). `codeql.yml` triggered on `pull_request` + a weekly Monday 06:00 schedule and nothing else. A PR analysis reports against the PR's MERGE COMMIT, and GitHub tracks code-scanning alerts PER REF — so `master` was analysed once a week: merged code sat up to 7 days unscanned, and an alert a merge actually FIXED stayed open on master until the next Monday. Measured, not reasoned: of **1,122 CodeQL runs, ZERO carried `event: push`**, and the only master analyses were scheduled ones. A `push: branches: [master]` trigger closes it; the schedule STAYS (it re-tests UNCHANGED code against queries published since, which push never does) and so does `pull_request` (catching a finding before it merges is the point of the PR check). **`security.yml` had the IDENTICAL gap and is fixed the same way** — Semgrep's findings reach the same Security tab through `codeql-action/upload-sarif`, and of 1,123 runs of that workflow ZERO carried `event: push` either. Cost measured before adding rather than assumed: Gitleaks 16s (full history and all), Semgrep 90s, CodeQL 3.3 min, all free on a public repo; and neither can disturb the deploy — `codeql.yml`/`security.yml` carry no concurrency block, `ci.yml`'s group is per-workflow, and build-deploy's `needs` are all internal to `ci.yml`. `codeqlPushTrigger.test.js` DERIVES its subject from BOTH publishing actions (`analyze` and `upload-sarif`) and its branch from `ci.yml`'s own deploy ref, so nothing can start publishing to code scanning without the trigger and the two cannot drift; the floor is **2**, not 1, or either file could lose coverage while the suite stayed green on the other. Comments are stripped before the match — these workflows now explain their own triggers in prose that NAMES the actions, and mutation showed that unstripped, a bare comment drags an unrelated workflow into the subject and fabricates a demand on it. **What it still does not cover**: a scanner that reports by FAILING ITS JOB instead of uploading SARIF (the Gitleaks job — `contents: read`, no `security-events: write`, so it creates no alerts and nothing about it can go stale on a ref). FOUND WHILE VERIFYING SOMETHING ELSE, which is the reusable part: I planned to read a dismissed alert's state off "the master run's fresh CodeQL analysis", and reading the triggers first is what showed no such analysis existed. That check would have found no CodeQL on the run and been one inference from reporting "nothing red, therefore confirmed" — a conclusion manufactured from an absence. **A missing mechanism and a passing mechanism look identical from the outside**; confirm the mechanism runs before reading its silence as a result.
...
**Eight "Clear text storage of sensitive information" alerts are scanner false positives and are dismissed as such** — #55 (`GradTab.tsx`), #56 (`HomeTab.tsx`), #58 + #66 (`useDailySession.ts`), #59 (`sessionSignal.ts`), #60 (`sessionCategory.ts`), #62 (`GrammarDiagnosisScreen.tsx`), #78 (`dailySessionStore.ts`, 2026-09-22 — below). Three heuristics misfire:
