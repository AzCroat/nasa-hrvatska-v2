...
function App() {
  ...
  const setScr = useCallback(
    ...
    // The exhaustive-deps suppression that used to sit here is gone: the body
    // referenced the component-scope `_RESTORE_SAFE` and `_TAB_FOR_SCR`, which
    // are module constants now, so `[navigate]` is genuinely complete.
    [navigate],
  );
  ...
  useEffect(() => {
    ...
    if (tabForPath) {
      ...
      _setCurrentScreen(last && RESTORE_SAFE_SCREENS.has(last) ? last : 'dashboard');
      return;
    }
