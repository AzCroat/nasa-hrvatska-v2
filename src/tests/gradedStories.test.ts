...
describe('graded stories — structural integrity', () => {
  ...
  it('per-level coverage never regresses below the 2026-07 expansion floor', () => {
    ...
    expect(byLevel['B2'] ?? 0).toBeGreaterThanOrEqual(27);
    expect(byLevel['C1'] ?? 0).toBeGreaterThanOrEqual(26);
    expect(byLevel['C2'] ?? 0).toBeGreaterThanOrEqual(22);
    expect(stories.length).toBeGreaterThanOrEqual(177);
  });
  ...
  it('per-level Croatian word volume never regresses below the tranche-1 floor', () => {
    ...
    expect(words['B2'] ?? 0).toBeGreaterThanOrEqual(10000);
    expect(words['C1'] ?? 0).toBeGreaterThanOrEqual(10000);
    expect(words['C2'] ?? 0).toBeGreaterThanOrEqual(10000);
  });
