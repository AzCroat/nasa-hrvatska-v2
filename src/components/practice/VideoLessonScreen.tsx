...
export default function VideoLessonScreen({ goBack, award }: VideoLessonProps) {
  ...
  // The MILDEST of the four and still worth fixing: this screen has a level
  // picker (`setLevel`), so the learner can override it — the stale value was a
  // wrong DEFAULT rather than a trap. Same one-line fix.
  const [level, setLevel] = useState(() => getGenerationCefr() as string);
