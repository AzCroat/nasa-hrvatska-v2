...
export interface CefrPoolEntry {
  ...
  adaptive?: boolean;
  /**
   * Comprehension slot (2026-09-04): the activity's CONTENT is generated by an
   * AI endpoint at request time rather than authored. The guaranteed input slot
   * (P2.8) prefers authored input — the same zero-AI-by-default posture as the
   * P2.4 conversation anchor — and takes a generated entry only when no authored
   * one is eligible. Nothing else reads this flag; it says nothing about cost
   * gating, which the budget governor owns. Every screen tagged here must call
   * an AI endpoint (pinned by sessionInputSlot.test.ts against the component
   * source), so the flag cannot drift into "things that feel AI-ish".
   */
  generated?: boolean;
}
